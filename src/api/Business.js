...
export function useGetBusines(BusinessId) {
  const URL = BusinessId
    ? [endpoints.Business.details, { params: { BusinessId } }]
    : null;
    ...
    await Promise.all(
      businessIds.map(async (businessId) => {
        try {
          ...
          if (success) {
            return deletedIds.push(businessId);
          } else {
            return  failedIds.push({ businessId, message: result.message });
          }
        } catch (error) {
          return failedIds.push({ businessId, message: error.errors });
        }
      })
    );
...
//   };
//   const URL ='https://tapis.ma-moh.com//api/business/create'; // Assuming this is correct

//   try {
//     ('Request Payload:', formData);

//     const response = await axios.post('https://tapis.ma-moh.com/api/images/create', formData, {
//       headers: {
...
export const createBusiness = async (formData, auth) => {
  try {
    const result = await axios.post(
      "https://tapis.ma-moh.com/api/business/create",
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
