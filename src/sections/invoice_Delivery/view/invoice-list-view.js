...
import { useGetReports, useGetReportsDelivery, useGetStatics, useGetStaticsDelivery, useGetStatusorder } from 'src/api/blog';
...
export default function InvoiceListView() {
  ...
  const TABLE_HEAD = [
    ...
    { id: 'status', label: t('Delivery Status') },
    { id: '' },
  ];
  ...
  const { reports, meta, reportsLoading, reportsEmpty } = useGetReportsDelivery();
  const { orderStatus } = useGetStatusorder();
  const { orderStatic } = useGetStaticsDelivery();
  ...
  const getInvoiceLength = (status) => {
    return reports.filter((item) => item?.orders_status?.delivery_display_name === status).length;
  };
  ...
  const TABS = [
    { value: 'all', label: i18n.language === 'ar' ? 'الكل' : 'All', count: reports?.length },
  ...
  const handleViewRow = useCallback(
    (id) => {
        router.push(paths.dashboard.invoiceDelivery.details(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: t('Invoices Delivery'),
              // href: paths.dashboard.invoiceDelivery.root,
            },
            {
              name: t('List'),
            },
  ...
  <InvoiceAnalytic
    key={index}
    title={i18n.language === 'ar' ?status?.translations[0]?.delivery_display_name:status?.translations[1]?.delivery_display_name}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (status !== 'all') {
    inputData = inputData.filter((invoice) => invoice?.orders_status?.delivery_display_name === status);
  }
