...
export default function InvoiceListView() {
  ...
  const { reports, meta, reportsLoading, reportsEmpty } = useGetReports(setidOFAll,dispalyids,8,  table.page + 1,
    table.rowsPerPage);
 
  const { orderStatus } = useGetStatusorder(8);
  const { orderStatic } = useGetStatics(8);
  ...
  <InvoiceAnalytic
    ...
    icon={status?.status_id===27? 'solar:sort-by-time-bold-duotone': status?.status_id===2 ? 'solar:file-check-bold-duotone':  status?.status_id===23 ? 'solar:file-check-bold-duotone' : status?.status_id===4 ? 'solar:bill-list-bold-duotone' :  status?.status_id===29 ? 'fluent:call-end-28-filled' :  status?.status_id===35 ? 'solar:file-corrupted-bold-duotone' : 'solar:file-corrupted-bold-duotone'}
    color={status?.status_id===27? theme.palette.info.main : status?.status_id===2 ? theme.palette.success.main:  status?.status_id===23 ? theme.palette.warning.main : status?.status_id===4 ? theme.palette.secondary.main :  status?.status_id===29 ? theme.palette.primary.main :  status?.status_id===35 ? theme.palette.error.main : theme.palette.text.secondary}
  />
))}
