...
export default function ChatNav({ loading, contacts, selectedConversationId }) {
  ...
  const handleSearchContacts = useCallback((inputValue) => {
    ...
    if (inputValue) {
      const results = contacts.filter((contact) =>
        contact.item_name.toLowerCase().includes(inputValue.toLowerCase())
      );
      setSearchContacts((prevState) => ({
        ...prevState,
        results,
  ...
  const handleClickResult = useCallback((result) => {
    handleClickAwaySearch();
     router.push(`/dashboard/chat?id=${result.chat_with_id}`);
  ...
  const renderContent = (
    <>
      ...
      <Scrollbar sx={{ pb: 1 }}>
        ...
        {/* Always render the contacts list */}
        {!collapseDesktop && !searchContacts.query && renderList}
      </Scrollbar>
    </>
  );
