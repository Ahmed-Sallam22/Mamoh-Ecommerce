...
export default function BusinessListView() {
  const {t}=useLocales()
  const TABLE_HEAD = [
    { id: "name", label: t("Store"), maxWidth: 40 },
    { id: "status", label: t("Status"), width: 110 },
    { id: "address", label: t("Address"), width: 140 },
    { id: "country", label: t("Country"), width: 140 },
    { id: "mobile_number", label: t("Mobile_Number"), width: 140 },
  ...
  const { Business, meta, BusinessLoading, BusinessEmpty } = useGetBusiness(
    table.page + 1,
    table.rowsPerPage
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    enqueueSnackbar(" Stores Deleted successfully!");
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Stores.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Stores.details(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.Stores.new}
              variant="contained"
      ...
      <ConfirmDialog
        ...
        content={
          <>
            ...
            <strong> {table.selected.length} </strong> {t("items")}?
          </>
        }
        action={
          <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (Business) =>
        Business.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
