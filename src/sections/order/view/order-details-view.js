        ...
        <Container maxWidth={settings.themeStretch ? false : "lg"}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid xs={12} md={4}>
              ...
              />
              {(order?.orders_status.id === 23 && order?.orders_status.id !== 14)&& (
        <>
          <LoadingButton
 onClick={async () => {
...
sx={{
      ...
      {(order?.orders_status.id === 27 && order?.orders_status.id !== 23) && (
        <LoadingButton
        onClick={async () => {
          setIsLoadingAccept(true); // Start loading when the button is clicked
          await handleChangeStatus(order?.orders_status?.owner_approve_step); // Call your API or function
  ...
  startIcon={isLoadingAccept && <CircularProgress size={24} color="inherit" />}
       ...
       {(order?.orders_status.id === 29 && order?.orders_status.id !== 27 &&order.pickup_type==1) && (
        <LoadingButton
        disabled
        sx={{ width: '100%', mt: 2 }}
          type="submit"
       ...
       {(order?.orders_status.id === 29 && order?.orders_status.id !== 27 &&order.pickup_type==2) && (
      <>
               <LoadingButton
 onClick={async () => {
  setIsLoadingAccept(true); // Start loading when the button is clicked
  ...
  setIsLoadingAccept(false); // Stop loading after the action is completed
    ...
    startIcon={isLoadingAccept && <CircularProgress size={24} color="inherit" />}
        ...
        sx={{
            ...
            loadingPosition="start" // Optional: Position of the spinner (start, end, bottom, top)
    startIcon={isLoadingDecline && <CircularProgress size={24} color="inherit" />}
          >
            {t('Decline')}
          </LoadingButton>
      </>
       ...
       {/* {(order?.orders_status.id === 35 && order?.orders_status.id !== 27) && (
       <LoadingButton
