...
export default function ProductListView() {
  ...
  const { Foodproducts, meta, FoodproductsLoading, FoodproductsEmpty } = useGetProducts(table.page + 1, table.rowsPerPage,filters.name);
  ...
  }, [Foodproducts, meta?.cuurent_page, meta?.per_page, meta]);
  ...
  const notFound = (!dataFiltered.length && canReset) || FoodproductsEmpty;
  ...
  const disincreaseQuantityBy1= useCallback(async (id, qty) => {
    // Calculate the new quantity ensuring it never goes below 0
